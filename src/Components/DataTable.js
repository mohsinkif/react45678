      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            >
              {(pathName === 'viewtransporter' && 'Transporter Name' ) || (pathName === 'viewharvesterdata' && 'Harvester Name') || (pathName === 'viewfarmer' && 'Farmer Name')}
            </th>
            <th
              scope="col"
              className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
            >
              {(pathName === 'viewtransporter' && 'Transporter Email' ) || (pathName === 'viewharvesterdata' && 'Harvester Email') || (pathName === 'viewfarmer' && 'Farmer Email')}
            </th>
            <th
              scope="col"
              className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
            >
              {(pathName === 'viewtransporter' && 'Transporter Phone' ) || (pathName === 'viewharvesterdata' && 'Harvester Phone') || (pathName === 'viewfarmer' && 'Farmer Phone')}
            </th>
          </tr>
        </thead>
          ...
          {data && data.length > 0 && data.map((eachEntry) => {
            return (
              <tr key={eachEntry.harvester_phone}>
                ...
                {pathName === 'viewtransporter' && eachEntry.transporter_name
  || pathName === 'viewharvesterdata' && eachEntry.harvester_name
  || pathName === 'viewfarmer' && eachEntry.farmername
  || 'Fallback Value'
}                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                {pathName === 'viewtransporter' && eachEntry.transporter_email
  || (pathName === 'viewharvesterdata' && eachEntry.transporter_email)
  || eachEntry.harvester_email
  || (pathName === 'viewfarmer' && eachEntry.farmeremail)
}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                { (pathName === 'viewtransporter' && eachEntry.transporter_phone)
  || (pathName === 'viewharvesterdata' && eachEntry.transporter_phone)
  || eachEntry.harvester_phone
  || (pathName === 'viewfarmer' && eachEntry.farmerphonenumber)
}
                </td>
